...
stage, and stopping immediately if a stage fails. Takes roughly 30-40
minutes end to end, most of it in the two cross-validated model bake-offs
(`model_bakeoff` and `forecast_bakeoff`).
...
`model_selection` → `predict` → `cross_cutting_prediction` → `assemble` →
`reporting` (each run as `python -m src.<module>`).
